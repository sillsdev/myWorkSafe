...
namespace UsbEject.Library
{
    ...
    public class Device : IComparable
    {
        ...
        internal Device(DeviceClass deviceClass, Native.SP_DEVINFO_DATA deviceInfoData, string path, int deviceNumber)
        {
            if (deviceClass == null)
		...
		public string DeviceId //PNPDeviceID
		{
			get
			{
				string deviceId = "";
				StringBuilder stringBuilder = new StringBuilder(1024);
				int hr = Native.CM_Get_Device_ID(InstanceHandle, stringBuilder, stringBuilder.Capacity, 0);
				if (hr != 0)
        ...
        public string Eject(bool allowUI)
        {
            foreach (Device device in RemovableDevices)
            {
                ...
                else
                {
					...
					//if there was some problem other than a veto
					if (hr != 0)
						return "Could not eject (hr=" + hr + ")";
                }
